/**
 * RoomClientV2 - New RoomClient using the layered architecture
 *
...
const logger = createModuleLogger('room-client-v2');

/**
 * Room client options
 */
export interface RoomClientV2Options {
  /**
 ...
 * Room events
 */
export interface RoomClientV2Events {
  /**
...
export class RoomClientV2 {
  ...
  private eventListeners: Map<
    keyof RoomClientV2Events,
    Set<RoomClientV2Events[keyof RoomClientV2Events]>
  > = new Map();

  constructor(private readonly options: RoomClientV2Options) {
   ...
   * Add event listener
   */
  on<K extends keyof RoomClientV2Events>(event: K, listener: RoomClientV2Events[K]): void {
   ...
   */
  off<K extends keyof RoomClientV2Events>(event: K, listener: RoomClientV2Events[K]): void {
   ...
   * Emit event
   */
  private emit<K extends keyof RoomClientV2Events>(
    event: K,
    data: Parameters<RoomClientV2Events[K]>[0]
  ): void {
